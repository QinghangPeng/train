...
public class Graph {
     ...
     * 返回ch位置
     */
    public int getPosition(String currentData) {
        for(int i=0; i<mVexs.length; i++) {
            if(Objects.equals(mVexs[i].data,currentData)) {
                return i;
            }
        }
        return -1;
     ...
     */
    public void print() {
        System.out.printf("List FindALlPath:\n");
        for (int i = 0; i < mVexs.length; i++) {
    ...
    public void prim(int start) {
        ...
        // 打印最小生成树
        System.out.printf("PRIM(%s)=%d: ", mVexs[start].data, sum);
        for (i = 0; i < index; i++) {
     ...
     */
    public String dijkstra(int vs, int[] prev, int[] dist,String targetStation,String targetLine) {
        String result = "";
        // flag[i]=true表示"顶点vs"到"顶点i"的最短路径已成功获取。
        boolean[] flag = new boolean[mVexs.length];

        int targetStationIndex = getPosition(targetStation);
        ...
        for (int i = 1; i < mVexs.length; i++) {
            ...
            if (StringUtils.isNotBlank(targetStation) && Objects.equals(mVexs[k].data,targetStation)){
                result = mVexs[k].data;
                break;
            }

            if (StringUtils.isNotBlank(targetLine) && Objects.equals(mVexs[k].data.substring(0,2),targetLine)){
                result = mVexs[k].data;
                break;
            }
        }
...
//        }
    ...
    public static void main(String[] args) {
        ...
        list.add(new EData("A","B",5));
        list.add(new EData("B","C",4));
        list.add(new EData("C","D",8));
        list.add(new EData("D","C",8));
        list.add(new EData("D","E",6));
        list.add(new EData("A","D",5));
        list.add(new EData("C","E",2));
        list.add(new EData("E","B",3));
        list.add(new EData("A","E",7));
        EData[] eDatas = list.stream().toArray(EData[]::new);
        Graph graph = new Graph(vexs,eDatas);
        graph.print();
//        graph.BFS();
    }
}
